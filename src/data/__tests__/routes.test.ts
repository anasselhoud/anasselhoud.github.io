...
describe('routes', () => {
  ...
  it('has unique paths among navigation routes', () => {
    // The index route (see "has exactly one index route" above) is a
    // branding entry that intentionally shares '/' with the "Home" nav
    // link — it renders elsewhere (e.g. the mobile menu's name header),
    // not as a second nav item, so it is excluded from this check.
